//============================================================================
// serial_port.cpp - Implements an API for raw serial I/O services
//============================================================================
#include <unistd.h>
#include <fcntl.h>
#include <termios.h>
...
//============================================================================
CSerialPort::CSerialPort() {fd_ = -1; default_timeout_ms_ = SP_NO_TIMEOUT;}
//============================================================================
...
//============================================================================
// setDefaultReadTimeout() - Sets the default timeout for all operations that
//                           read data from the serial port.   This includes
//                           "read()", "getLine()", and "getChar()"
//============================================================================
void CSerialPort::setDefaultReadTimeout(uint32_t milliseconds)
{
    default_timeout_ms_ = milliseconds;
}
//============================================================================
...
//=============================================================================
// baudToConstant() - Translates an integer baud rate to one of the speed_t
//                    constants
//
...
//=============================================================================
speed_t CSerialPort::baudToConstant(uint32_t baud)
{
    switch (baud)
    {
...
//============================================================================
bool CSerialPort::open(std::string device, uint32_t baud)
{
    termios tio;
    ...
    speed_t speed = baudToConstant(baud);
...
//============================================================================
// dataIsAvailable() - Waits for data to become available for reading on the
//                     serial port.
//
...
//============================================================================
bool CSerialPort::dataIsAvailable(int timeout_ms)
{
    fd_set  rfds;
    timeval timeout;

    // If we're supposed to use the default timeout, do so
    if (timeout_ms == SP_DEFAULT_TIMEOUT) timeout_ms = default_timeout_ms_;
    ...
    int status = select(fd_+1, &rfds, NULL, NULL, pTimeout);
...
//============================================================================
// drainInput() - Drains all data from the serial port and throws it away
//============================================================================
void CSerialPort::drainInput(int timeout_ms)
{
    char c;

    // Read in and throw away data until the line goes quiet for awhile
    while (dataIsAvailable(timeout_ms)) ::read(fd_, &c, 1);
}
//============================================================================


//============================================================================
// GetLine() - Fetches a line from the serial port.   Throws away carriage
//             returns and strips off the terminating line-feed
//============================================================================
bool CSerialPort::getLine(void* buffer, int timeout_ms)
{
    ...
    while (true)
    {
        // Read a single character from the serial port using GetChar
        int c = getChar(timeout_ms);
...
void CSerialPort::printf(const char* fmt, ...)
{
    ...
    // Format string "buffer" according to the callers fmt and arguments
...
//============================================================================
// putLine() - Sends a line of text to the serial port.
//============================================================================
void CSerialPort::putLine(const void* text)
{
...
//============================================================================
// getChar() - Fetches one byte from the serial port
//
...
//============================================================================
int CSerialPort::getChar(int timeout_ms)
{
    unsigned char c;
    ...
    // occured.
    if (!dataIsAvailable(timeout_ms)) return -1;

    // Read a single character from the serial port
    ::read(fd_, &c, 1);

    // If we are supposed to display our output, do so
    if (sniff_) printf("%c", c);
...
//============================================================================
// putChar() - Writes a single byte to the serial port
//============================================================================
void CSerialPort::putChar(int byte)
{
